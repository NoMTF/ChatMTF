...
import com.inspiredandroid.kai.data.OcrService
...
import com.inspiredandroid.kai.data.StructuredDataStoreHolder
import com.inspiredandroid.kai.data.TaskScheduler
import com.inspiredandroid.kai.data.TaskStore
import com.inspiredandroid.kai.data.ToolExecutor
import com.inspiredandroid.kai.data.createStructuredDataStore
...
val appModule = module {
    ...
    single {
        StructuredDataStoreHolder(
            createStructuredDataStore()?.also { store ->
                store.migrateFromSettingsIfNeeded(get())
            },
        )
    }
    single<Requests> {
    ...
    single<ConversationStorage> {
        ConversationStorage(get(), get<StructuredDataStoreHolder>().store)
    }
    single<ToolExecutor> {
        ToolExecutor()
    }
    single<MemoryStore> {
        MemoryStore(get(), get<StructuredDataStoreHolder>().store)
    }
    single<TaskStore> {
        TaskStore(get(), get<StructuredDataStoreHolder>().store)
    }
    single<EmailStore> {
        EmailStore(get(), get<StructuredDataStoreHolder>().store)
    }
    single<EmailPoller> {
        EmailPoller(get<EmailStore>())
    }
    single<SmsStore> {
        SmsStore(get(), get<StructuredDataStoreHolder>().store)
    }
    single<SmsPoller> {
        SmsPoller(get<SmsStore>(), get<SmsReader>())
    }
    single<SmsDraftStore> {
        SmsDraftStore(get(), get<StructuredDataStoreHolder>().store)
    }
    single<NotificationStore> {
        NotificationStore(get(), get<StructuredDataStoreHolder>().store)
    }
    single<OcrService> {
        OcrService(get(), get())
    }
