...
import chatmtf.composeapp.generated.resources.tool_check_email_description
import chatmtf.composeapp.generated.resources.tool_check_email_name
import chatmtf.composeapp.generated.resources.tool_compose_email_description
import chatmtf.composeapp.generated.resources.tool_compose_email_name
import chatmtf.composeapp.generated.resources.tool_read_email_description
import chatmtf.composeapp.generated.resources.tool_read_email_name
import chatmtf.composeapp.generated.resources.tool_reply_email_description
import chatmtf.composeapp.generated.resources.tool_reply_email_name
import chatmtf.composeapp.generated.resources.tool_search_email_description
import chatmtf.composeapp.generated.resources.tool_search_email_name
import chatmtf.composeapp.generated.resources.tool_setup_email_description
import chatmtf.composeapp.generated.resources.tool_setup_email_name
...
object EmailTools {
    ...
    fun checkEmailTool(emailStore: EmailStore) = object : Tool {
        override val schema = ToolSchema(
            name = "check_email",
            description = "List emails that have arrived since the last time ChatMTF surfaced new mail to the user. ChatMTF tracks delivery internally and ignores the provider's read flag, so an email shows up here at most once whether it was first seen via heartbeat or a previous check_email. To find an email that's already been surfaced (or any older message), use search_email with `from` / `subject` / `since`. If multiple accounts are connected, checks all of them.",
            parameters = mapOf(
        ...
        override suspend fun execute(args: Map<String, Any>): Any {
            ...
            return buildMap {
                ...
                if (allEmails.isEmpty()) {
                    put(
                        "hint",
                        "No new emails since the last delivery. To find a message ChatMTF has already surfaced (or any older email), call search_email with the account_id from `accounts` and a `from` / `subject` / `since` filter.",
                    )
                }
            }
        }
