...
internal const val DEFAULT_SOUL_MAINTENANCE_SECTION =
    "## Soul Self-Maintenance\n" +
        "You may use read_soul, update_soul, append_soul, and reset_soul to maintain your own Soul/system prompt. " +
        "Use them only for durable identity, behavior, boundary, or operating-rule changes, not for ordinary memories or temporary task context. " +
        "Small stable additions should use append_soul; broad rewrites should use update_soul after reading the current Soul. " +
        "Avoid meaningless frequent edits. If you make a major personality or boundary change, mention it briefly in your reply."
...
internal const val DEFAULT_SEGMENTED_SENDING_SECTION =
    "## Segmented Sending\n" +
        ...
        "Do not use blank lines as a splitting signal. Use at most 8 short segments, and only when each segment can stand alone."
...
internal fun buildChatSystemPrompt(
    ...
    segmentedSendingEnabled: Boolean = true,
    ...
    if (hasTools) {
        if (isNotEmpty()) append("\n\n")
        append(DEFAULT_SOUL_MAINTENANCE_SECTION)
    }

    if (segmentedSendingEnabled) {
        if (isNotEmpty()) append("\n\n")
        append(DEFAULT_SEGMENTED_SENDING_SECTION)
    }
