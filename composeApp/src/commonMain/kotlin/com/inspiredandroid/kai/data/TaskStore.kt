...
class TaskStore(
    ...
    private fun loadTasks(): MutableList<ScheduledTask> = try {
        val raw = structuredDataStore?.readJson(StructuredDataKeys.TASKS) ?: appSettings.getScheduledTasksJson()
        val decoded = json.decodeFromString<List<ScheduledTask>>(raw)
    ...
    private fun saveTasks(tasks: List<ScheduledTask>) {
        ...
        runCatching { structuredDataStore?.writeJson(StructuredDataKeys.TASKS, data) }
            .onFailure { println("TaskStore: SQLite persist failed: ${it.message}") }
        appSettings.setScheduledTasksJson(data)
    }
