...
class NotificationStore(
    ...
    private val pendingQueue = PendingQueue<NotificationRecord, String>(
        ...
        structuredKey = StructuredDataKeys.NOTIFICATION_PENDING,
    )
    ...
    fun getStore(): List<NotificationRecord> {
        val raw = structuredDataStore?.readJson(StructuredDataKeys.NOTIFICATION_STORE) ?: appSettings.getNotificationsStoreJson()
    ...
    private fun saveStore(records: List<NotificationRecord>) {
        val data = json.encodeToString(records)
        runCatching { structuredDataStore?.writeJson(StructuredDataKeys.NOTIFICATION_STORE, data) }
            .onFailure { println("NotificationStore: SQLite persist failed: ${it.message}") }
        appSettings.setNotificationsStoreJson(data)
    }
