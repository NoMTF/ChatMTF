...
class MemoryStore(
    ...
    private fun loadMemories(): MutableList<MemoryEntry> {
        val raw = structuredDataStore?.readJson(StructuredDataKeys.MEMORIES) ?: appSettings.getMemoriesJson()
    ...
    private fun saveMemories(memories: List<MemoryEntry>) {
        val data = json.encodeToString(memories)
        runCatching { structuredDataStore?.writeJson(StructuredDataKeys.MEMORIES, data) }
            .onFailure { println("MemoryStore: SQLite persist failed: ${it.message}") }
        appSettings.setMemoriesJson(data)
    }
